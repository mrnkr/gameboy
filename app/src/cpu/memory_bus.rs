...
impl MemoryBus {
    ...
    pub fn write_word(&mut self, address: u16, new_value: u16) {
        let least_significant_byte = (new_value & 0x00FF) as u8;
        let most_significant_byte = ((new_value & 0xFF00) >> 8) as u8;
        self.write_byte(address, least_significant_byte);
        self.write_byte(address + 1, most_significant_byte);
    }
}
